...
###		MODEL INPUTS 
#################################################################################
ntrials = 500						# number of iterations for simulation


tstep = 365							# timestep in days
nstocks = 5						# number of regions
##################################################################
## NCOHORTS==NYRS, can't deal with issues of no releases yet
###############################################################
ncohorts = 5						# years of tag releases
nyrs  = 5							# yrs of recaps

nages = 5							    # number of age classes, the terminal age class is a plus group; each 'at-age' input below must have this many entries per region
...
marks = c(1000, 1000, 1000, 1000, 1000)		# number of marked individuals per region per year
...
spawn_par = c(1, 1)					# define the spawning season distribution parameters, for uniform start and end day of year, for normal mean and standard dev, for poisson mean
...
tag_dist = c(0, 0, 0, 0,0)				# define the distribution of tagging effort in region 1, 0 = uniform, 1 = normal, 2 = poisson
tag_par = matrix(c(
		1, 1,					# define the tagging season distribution parameters each region, for uniform start and end day of year, for normal mean and standard dev, for poisson mean
		1, 1, 
		1, 1,
		1, 1,
		1, 1
),ncol=2,byrow=TRUE)
fishery_dist = c(0, 0, 0, 0, 0, 0)					# define the distribution of fishing effort in each region, 0 = uniform, 1 = normal, 2 = poisson
fishery_par = matrix(c(
			1, 365,				# define the fishing distribution parameters for each region , for uniform start and end day of year, for normal mean and standard dev, for poisson mean
			1, 365,
			1, 365,
			1, 365,
			1, 365
...
M = c(0.365,0.306,0.274,0.255,0.243)			# annual natural mortality-at-age
F = read.csv('F_matrix.csv',row.names=1)		# annual regional fishing mortality-at-age
tag_loss1 = c(0, 0, 0, 0, 0)			# type 1 discrete tag loss from the handling process
tag_loss2 = 0						# type 2 chronic tag loss from tissue deterioration, biofouling etc.
handling_mort = c(0, 0, 0, 0, 0)			# discrete handling mortality rates
report_rates = c(0.25, 0.25, 0.25, 0.25, 0.25)			# fleet reporting rates
...
residency = c(.6, .6, .6, .6, .6)						# rate of residency of fish in home region (in numerical order by region), only used when mixing = 2 or 3
