...
fate=function(mark_region,cohort)
	...
	M_1=M[max(1,age1)]*(tstep/365-(time1-(tstep/365)*trunc(365*time1/tstep)))
	F_1=F[(nyrs*(region1-1)+cohort),trunc(max(1,age1))]*sum(effort(region1)[time1:tstep])
	if(model_type == 1)
		{
	...
	cap_hist[1,]=c(age1,cohort,time1,age1,mark_region,region1,handle1,doubletag1,1-loss1,1-loss2,lambda1,survive1,capture1,report1)	
	cap_hist[,1]=round(age1,2)
	cap_hist[,2]=cohort
	cap_hist[,3]=round(time1+(0:(timesteps-1))*tstep/365,2)
	cap_hist[,4]=round(age1+cap_hist[,3],2)
...
pb <- winProgressBar(title="Simulation Progress Bar - Initial Run", label="0% done", min=0, max=100, initial=0)
...
write.csv(cbind(tag_loss,tag_loss_est=tag_loss_vector),"tag_loss.csv",row.names=FALSE)
